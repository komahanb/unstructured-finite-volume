...
contains
  ...
  subroutine check_global_solve(nfail)
    ...
    call solver % state(shifted, g, vs, n_vs)
    ...
    call solver % apply(g, solver % bind([rhs]), sol)
  ...
  subroutine check_host_is_load_bearing(nfail)
    ...
    call solver_g % state(shifted, g, vs, n_vs)
    call solver_alt % state(shifted, g_alt, vs_alt, n_vs_alt)
