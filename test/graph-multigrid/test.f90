...
contains
  ...
  subroutine check_two_roads_one_answer(nfail)
    ...
    call direct % state(chain_statement(m), m, m % vertex_set(), &
         & m % num_vertices())
    ...
    !
    ! STATE IS RE-ENTERABLE. Newton calls it once per iteration, and
    ! it declares the solver's number domain each time - which a graph
    ! refuses, because a graph signs ONCE. The old counted_set
    ! constructor minted a fresh number domain per state, so that is
