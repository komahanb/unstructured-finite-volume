program execution_independence
  ...
  use util_tally, only : tally_open, tally_close, tally_amount, tally_num_levels, linear_solves, newton_solves
...
contains
  ...
  subroutine bounded_case(case_index)
    ...
    do order = 1, 3
       top = order - 1
       before = counted_solves(linear_solves)
       ...
       solves_retained = counted_solves(linear_solves) - before
       ...
       if (order > 1) then
          ...
          do c = 0, blocks - 1
             ...
             solves_bounded = counted_solves(linear_solves) - before
  ...
  subroutine streamed_case(case_index, order, m, blocks, functional, designs, whole_table, whole_by_order, &
    ...
    before_newton = counted_solves(newton_solves)
    before_linear = counted_solves(linear_solves)
    call finish(other)
    march_newton = counted_solves(newton_solves) - before_newton
    march_linear = counted_solves(linear_solves) - before_linear
    ...
    newton_retained = counted_solves(newton_solves) - before_newton
    linear_retained = counted_solves(linear_solves) - before_linear
    ...
    do c = 0, blocks - 1
       ...
       before_linear = counted_solves(linear_solves)
       call finish(other)
       call check(nint(counted_solves(newton_solves) - before_newton) == nint(march_newton) .and. &
            & nint(counted_solves(linear_solves) - before_linear) == nint(march_linear), &
       ...
       call other % derivative(functional, order, reverse_pass, table, by_order=by_order, entries=entries, &
            & designs=designs, storage=bounded)
       extra_newton = counted_solves(newton_solves) - before_newton - newton_retained
       extra_linear = counted_solves(linear_solves) - before_linear - linear_retained
       call check(bounded % checkpoints == max(expected, 0), &
  ...
  real(dp) function counted_solves(event)
    integer, intent(in) :: event
    integer :: level, order
    counted_solves = 0.0_dp
    do level = 1, tally_num_levels()
       do order = 0, 3
          counted_solves = counted_solves + tally_amount(level, order, event)
       end do
    end do
