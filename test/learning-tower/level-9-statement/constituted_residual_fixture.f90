...
! literal, no 2*w - 6 anywhere. What it stores is the STATEMENT'S
! data: the GRAPH-OWNED flow (found by identity, refused if
! the graph does not own it - the external selector may be deallocated), the
...
module constituted_residual_fixture
  ...
  ! types with two names now, so nothing is renamed here.
  use graph_fractal        , only : graph
...
contains
  ...
  subroutine clr_apply(this, input_graph, inputs, output)
    ...
    call emit_real('residual', this % rows, this % n_rows, r, output)
