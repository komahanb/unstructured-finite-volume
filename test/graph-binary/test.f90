...
! level 1, phase 3).
!
! The CSR relation evaluates image and preimage as O(degree) slices,
! takes and returns members across any carrier representation -
! the sparse-source check is the inverse-map law - and the
! transpose is a referencing view: O(1) to make, ends swapped, its
! own identity. The involution (R^T)^T = R is checked the only way
! it is promised: by EXTENSION, tuples and domains, never by
! identity.
!
...
contains
  ...
  !===================================================================!
  ! The inclusion relation I_S <= S x A: the pair (s, s) for every
  ! member s of S, named after the two labels. The tuples are derived
  ! from S's own extent; the ambient is the signature's second domain.
  !===================================================================!

  type(csr_relation) function inclusion_relation(s, ambient, sets, labels) &
       & result(inclusion)
    ...
    inclusion = csr_relation(labels % label_of(s) // ' in ' // &
         &                   labels % label_of(ambient), s, ambient, table, sets)

  end function inclusion_relation
  ...
  !===================================================================!
  ! The CSR relation on cells x faces: fibres both ways, membership
  ...
  subroutine check_csr_contract(nfail)
    ...
    ! One duplicate in the input, on purpose.
    r = csr_relation('touches', cells, faces, &
         & reshape([1,1,  1,2,  2,2,  3,4,  1,1], [2, 5]), sets)

    call report(r % arity() .eq. 2, &
         & "arity two, defined once for the whole family", nfail)
    call report(r % num_tuples() .eq. 4, &
         & "a tuple input twice is in the relation once", nfail)
    ...
    call report(size(idx) .eq. 0, &
         & "and an unrelated face has an empty preimage too", nfail)
    ...
    call report(size(t, 2) .eq. 4, &
         & "the tuple table is the set, not the input", nfail)
  ...
  ! Rows are at local indices; inputs and results are
  ! members; local_index is the only translation, and outsiders have
  ! empty.
  !===================================================================!
  ...
  subroutine check_sparse_source(nfail)
    ...
    call r % image(30, idx)
    call report(size(idx) .eq. 0, &
         & "a member with no tuples has an empty image", nfail)

    call r % image(15, idx)
    call report(size(idx) .eq. 0, &
         & "an outsider has an empty image: relating to nothing is a result", &
         & nfail)
    ...
    call report(d % same_as(sensors), &
         & "the sparse slot returns its own declared domain", nfail)

  end subroutine check_sparse_source

  !===================================================================!
  ! The subobject as a relation: I_S <= S x A contains (s, s) for
  ! every member, total, functional, injective by construction -
  ! and it is an ordinary CSR relation, so the subset is a relation
  ! domain like any declared domain.
  !===================================================================!
  ...
  subroutine check_inclusion(nfail)
    ...
    call faces % declare()
    call sets % bind(faces, counted_set_representation(5))
    call boundary % declare()
    call sets       % bind(boundary, listed_set_representation([2, 5]))
    call inclusions % include_in(boundary, faces)

    call labels % bind(boundary, 'boundary')
    call labels % bind(faces, 'faces')
    inc = inclusion_relation(boundary, faces, sets, labels)

    call report(inc % arity() .eq. 2 .and. inc % num_tuples() .eq. 2, &
         & "the inclusion contains one tuple per subset member", nfail)
    ...
    d = inc % domain(1)
    call report(d % same_as(boundary), &
         & "the source is the subset itself", nfail)
    d = inc % domain(2)
    call report(d % same_as(faces), &
         & "the target is the ambient it was declared into", nfail)
  ...
  ! S's own extent and stores no translation. The DECLARED AMBIENT is
  ! stored by the SIGNATURE, and by nothing else.
  !
  ! Two ambients with identical extensions make the point exactly: two
  ! subsets of the same members, declared into A and into B, have the
  ...
  subroutine check_inclusion_provenance(nfail)
    ...
    ia = inclusion_relation(sa, a, sets, labels)
    ib = inclusion_relation(sb, b, sets, labels)
    ...
    call report(.not. ia % same_as(ib), &
         & "so the ambient is in the signature, never in the extension", nfail)

  end subroutine check_inclusion_provenance

  !===================================================================!
  ! The transpose view: O(1) to make, nothing copied, every result
  ...
  subroutine check_transpose_view(nfail)
    ...
    call report(r % materialized() .and. .not. t % materialized(), &
         & "the base is materialized; the view is not", &
         & nfail)
  ...
  ! enumerate differently; the exact test is
  !
  ...
  ! R by identity, because same_as compares tokens and no
  ...
  subroutine check_involution_is_extensional(nfail)
    ...
    passes = .true.
    do j = 1, size(rt, 2)
       passes = passes .and. tt % has(rt(:, j))
    end do
    call report(passes, &
         & "and contains every tuple of the base: equal as sets", nfail)
    ...
    db = r % domain(2)
    call report(da % same_as(db), &
         & "both slots return to the base's domains", nfail)
