...
\paragraph{The average step $S$} puts on each edge the average of its
two end values, or -- when a direction matters -- the value at one
end, chosen by the sign of the coefficient the operator carries:
\[
...
\paragraph{The fold step $D$} brings edge values back to the
...
One convention to fix now, because two are in use. The fold above is
...
the engine folds the other way -- in minus out -- because a balance
counts what a vertex \emph{gains}. The two folds differ by a sign and
...
of order $n-1$. The coefficient $c$ is applied once, at the outermost
step. (The order-$2$ recipe is what the literature calls the graph
...
\]
and the fold step subtracts them and divides by $m=h$:
\[
...
cell volume $V_v$  & vertex measure $m_v$ & the fold step \\
...
field, and the fold step -- the same $D$ as everywhere in this note,
with the $\pm 1$ as its per-edge coefficients -- collects the walk
around each face. The curl is the fold step on the border graph.
...
bare; weights ride along unchanged). Pair a vertex field $p$ with a
fold:
\[
\sum_v p_v\,(Dz)_v
...
D^{\!*} = -\,G, \qquad G^{\!*} = -\,D,
\]
and the one-sided average step's adjoint folds, unsigned, onto the
...
operator's adjoint samples from the \emph{other} end and folds the
...
restating. A quantity like an average must be folded across parts
...
average step $S$      & \texttt{average\_step}                & kernel \\
difference step $G$   & \texttt{difference\_step}             & kernel \\
fold step $D$         & \texttt{fold\_step}                   & kernel \\
