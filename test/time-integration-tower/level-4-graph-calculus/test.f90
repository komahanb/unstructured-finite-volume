...
program time_level_4
  ...
  type(graph)              :: q, t, e
  ! sources/sinks declare subobjects, and a subobject binds extent,
  ! label and inclusion together - so the set store, not the bare
  ! set map, is what this level owns.
  type(set_store)            :: sets
  ...
  call time_carriers(sets % set_map, q, t, e)
  tail = tail_relation(e, t, sets % set_map)
  head = head_relation(e, t, sets % set_map)
  a1   = derive_one_step_reach(tail, head, sets % set_map)
  a2   = derive_two_step_reach(a1, sets % set_map)
...
contains
  ...
  subroutine check_causal_ends(nfail)
    ...
    call sources(a1, sets, src)
    call sinks(a1, sets, snk)
    ...
    call report(sets % subobject_of(src, t) .and. sets % subobject_of(snk, t), &
  ...
  subroutine check_causal_reachability(nfail)
    ...
    call report(reachable(a1, sets % set_map, T0, T4), &
         & "t0 reaches t4 along the causal chain", nfail)
    call report(.not. reachable(a1, sets % set_map, T4, T0), &
         & "and t4 reaches t0 not at all: THE ASYMMETRY IS " // &
         & "STRUCTURAL, not a convention about which way to loop", &
         & nfail)

    call report(reachable(a1, sets % set_map, T2, T2), &
         & "every instant reaches itself by the zero-length path", &
         & nfail)
    call report(reachable(a1, sets % set_map, T1, T3) .and. &
         &      .not. reachable(a1, sets % set_map, T3, T1), &
  ...
  subroutine check_two_step_view(nfail)
    ...
    call report(.not. reachable(a2, sets % set_map, T0, T3), &
