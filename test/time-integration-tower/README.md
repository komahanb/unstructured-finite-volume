...
`src/operation_step.f90` and `src/operation_marching.f90`, named in the
table and the RED records below, were deleted in commit b9944c3 (the
march is a graph the driver evaluates). Levels 6-9 now read the
scheme's coefficients from `operation_family` and drive the march by
`operation_driver` over a bipartite step/state digraph
(`view_read_write`); the step residual and the rule at a step are
stated in `common/temporal_step_fixture.f90` (Level 6) and
`common/temporal_march_fixture.f90` (Level 8). Level 4 declares its
sources and sinks through `map_set_store`. The records below are
kept as written: they describe the production the tower was built
against.
