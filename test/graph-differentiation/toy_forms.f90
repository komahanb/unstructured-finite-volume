...
! linearization and the minimizer's stored inputs.
!
...
!
! xi is read from the binding of argument 2 when the caller binds
! it, and from the xi_default component otherwise. The chain rule
! tests bind both arguments; the tangent binds only the state, so
! the default keeps one operation usable by both.
!
...
module toy_differentiable_forms
  ...
  use operation_action, only : binding, bound_real_vector, is_bound
  ...
  !===================================================================!
  ! S_i(q, xi) = q_i^2 - xi, elementwise. max_degree is 8 because
  ! the tests request compositions up to degree 8; every partial of
...
contains
  ...
  !===================================================================!
  ! Read the state vector from the binding of argument 1, and xi
  ! from the binding of argument 2 when bound or from xi_default
  ! otherwise.
  !===================================================================!

  subroutine read_arguments(this, inputs, xi_default, q, xi)
    ...
    if (is_bound(inputs, this % argument(2))) then
       call bound_real_vector(inputs, this % argument(2), xv)
       xi = xv(1)
    else
       xi = xi_default
    end if
  ...
  ! stores the k-th scalar, every entry marked occupied.
  !===================================================================!
  ...
  subroutine quartic_apply(this, input_graph, inputs, output)
    ...
    if (present(inputs)) then
       call read_arguments(this, inputs, this % xi_default, q, xi)
  ...
  subroutine quartic_partial_action(this, input_graph, inputs, &
       & variations, output)
  ...
  subroutine power8_apply(this, input_graph, inputs, output)
  ...
  subroutine power8_partial_action(this, input_graph, inputs, &
       & variations, output)
  ...
  subroutine equilibrium_apply(this, input_graph, inputs, output)
  ...
  subroutine equilibrium_partial_action(this, input_graph, inputs, &
       & variations, output)
  ...
  subroutine linear_apply(this, input_graph, inputs, output)
    ...
    if (present(inputs)) then
       call bound_real_vector(inputs, this % argument(1), s)
    else
       allocate(s(input_graph % num_vertices()))
       s = 0.0_dp
    end if
