...
allowed_for() {
    case "$1" in
        ...
        #                            QUERIED
        #   map_set_store      granted only where a subobject is
        #                            DECLARED, provenance is ASSERTED,
        #                            or a transport is CALLED
        #
        # The store is the set map with the label map and the
        # inclusion map beside it (1167b53). Declaring a subobject
        # binds extension, label and embedding TOGETHER so no
        # half-described set escapes, and the transports read the
        # embedding through the store, so a caller of a transport
        # must supply the store even where it reads no label.
        ...
        # Calls partition_data and assemble_data - both declare
        # subobjects - and queries the domains it transports. It also DESCRIBES the
        # part carriers it transports onto, so it constructs
        # representations and earns the module that makes them.
        common/partitioned_shifted_laplacian_fixture.f90) echo "graph_fractal map_set_representation map_set_store operation_action view_directed relation_partition field_calculus view_directed_stored field_stored transform_partitioner transform_assembler shifted_laplacian_fixture" ;;
        ...
        #          Declares the owned, borrowed and overlap subobjects
        #          of a part's carrier - hence the store.
        level-4-graph-calculus) echo "partitioned_pde_assert chain_carriers_fixture chain_relations_fixture chain_algebra_fixture graph_fractal map_set_representation map_set map_set_store view_directed relation_partition relation_finitary relation_algebra relation_binary view_directed_stored transform_partitioner" ;;
        # ---- L5: + fields and transport. NO differential operator.
        #          The one level that both DECLARES a subset of its
        #          own and ASSERTS where a transported domain came from.
        level-5-field-calculus) echo "partitioned_pde_assert graph_fractal map_set_representation map_set_store view_directed relation_partition field_calculus view_directed_stored field_stored transform_partitioner transform_assembler" ;;
        # ---- L6: + the differential operator and its fixture.
        #          Still no solver. Transport declares subobjects;
        #          the refusal builds a foreign carrier, and is keyed
        #          below.
        level-6-discretization) echo "partitioned_pde_assert shifted_laplacian_fixture graph_fractal map_set_representation map_set_store view_directed relation_partition field_calculus view_directed_stored field_stored transform_partitioner transform_assembler operation_differential" ;;
...
if [ "$1" = "--selftest" ]; then
    ...
    refuses common/shifted_laplacian_fixture.f90 map_set_store

    # Transport reads the embedding through the store, so the sources
    # that transport or declare subobjects get it and the sources
    # that do not are refused.
    permits common/partitioned_shifted_laplacian_fixture.f90 map_set_representation
    permits common/partitioned_shifted_laplacian_fixture.f90 map_set_store
    refuses common/partitioned_shifted_laplacian_fixture.f90 map_inclusion
    refuses common/partitioned_shifted_laplacian_fixture.f90 map_label
    permits level-4-graph-calculus map_set_store
    refuses level-4-graph-calculus map_inclusion
    ...
    refuses level-7-minimization map_set_store
