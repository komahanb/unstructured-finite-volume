...
> **Migration note (2026-09-04, src at graph-time-integrators).** The
> prose below records the tower as first written against the src of
> a4e8f30~1. Levels 6-9 now read the same structures through the
> current src: `stencil % dependencies()` is the stored component
> `stencil % pattern` (the accessor left in src commit 9fd1732);
> `step % dependencies()` is `family % block_connectivity(nd, n)` over
> the shortest block the row pattern fits in (`operation_step` became
> `operation_family` and `operation_driver` in b9944c3; a family takes
> no action and no step size); `minimizer % attach` is
> `minimizer % state`, `minimizer % diagonal` is `block_diagonal` at
> block width one. Level 6's check that two steps around actions of
> different state sparsity return one motif has no expression in the
> current src, because a family is not built around an action; that
> check and `diagonal_stencil`/`bdf2_around` are removed.
