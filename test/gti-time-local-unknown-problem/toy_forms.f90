...
module gti_toy_forms
  ...
  !===================================================================!
  ! The clock: R = [t], the witness that the requested evaluation
  ! time truly reaches the form. It reads nothing else of the
  ! point, and its partials are those of a linear function of t.
  !===================================================================!

  type, extends(gti_differentiable_form) :: toy_clock_form

   contains

     procedure :: name             => clock_name
     procedure :: input_signature  => clock_input_signature
     procedure :: output_signature => clock_output_signature
     procedure :: max_degree       => clock_max_degree
     procedure :: value            => clock_value
     procedure :: partial_action   => clock_partial_action

  end type toy_clock_form
...
contains
  ...
  pure function clock_name(this) result(name)

    class(toy_clock_form), intent(in) :: this
    character(len=:), allocatable :: name

    name = 'toy clock'

  end function clock_name

  pure function clock_input_signature(this) result(signature)

    class(toy_clock_form), intent(in) :: this
    integer, allocatable :: signature(:)

    signature = [GTI_ARG_TIME]

  end function clock_input_signature

  pure function clock_output_signature(this) result(signature)
  ...
  end function clock_output_signature

  pure function clock_max_degree(this) result(degree)
  ...
  end function clock_max_degree

  subroutine clock_value(this, point, output)

    class(toy_clock_form)     , intent(in)    :: this
    type(gti_evaluation_point), intent(in)    :: point
    type(gti_value_buffer)    , intent(inout) :: output

    call output % set_real([point % time])

  end subroutine clock_value

  subroutine clock_partial_action(this, point, request, directions, output)
    ...
    case (1)
       ...
       if (request % argument_kind(1) == GTI_ARG_TIME) then
          call directions(1) % values % get_real(v1)
  ...
  end subroutine clock_partial_action
