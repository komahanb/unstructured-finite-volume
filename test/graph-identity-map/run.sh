...
if command -v valgrind >/dev/null 2>&1; then
    ...
    # native run. Before the storage law the lifetime check reported
    # 170 invalid reads from 48 contexts and still printed ALL
    # PROPOSITIONS HOLD.
    for check in lifetime labels; do
        if valgrind --error-exitcode=9 --leak-check=full \
                    --errors-for-leak-kinds=definite \
                    ./$check >valgrind.out 2>&1; then
            echo " PASS : $check reads no storage it does not own"
        else
            echo " FAIL : $check still references its keys"
