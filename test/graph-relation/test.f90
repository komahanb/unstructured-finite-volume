...
! no graph appears anywhere in this file. The checks are the
...
! as two views of one primitive, the boundary written with no
...
contains
  ...
  ! membership, tuples verbatim, and a signature that is compared by
  ! structural identity.
  !===================================================================!
  ...
  subroutine check_incidence_contract(nfail)
    ...
    call report(tk % declared(), &
         & "a declared relation stores a declared token", nfail)
    ...
    call report(r % num_tuples() .eq. 4, &
         & "the count is the tuples input", nfail)
    ...
    call report(size(t, 1) .eq. 2 .and. size(t, 2) .eq. 4 .and. &
         ...
         & "tuples return as input, in input order", nfail)

    d = r % domain(1)
    call report(d % same_as(cells), &
         & "slot one is the cells domain, by identity", nfail)
    call report(.not. d % same_as(faces), &
         & "and is not the faces domain, whatever the sizes say", nfail)
    d = r % domain(2)
    call report(d % same_as(faces), &
         & "slot two is the faces domain", nfail)

  end subroutine check_incidence_contract

  !===================================================================!
  ! A relation is a set, not a bag: a tuple input twice is in
  ! the relation once, and everything the interface returns - the
  ! count, the table, membership - obeys set semantics. Parallel
  ...
  !===================================================================!
  ! Carriers obey set semantics too: a member input twice is in
  ...
  subroutine check_carrier_set_semantics(nfail)
    ...
    call report(sets % num_members_of(dup) .eq. 3, &
         & "a member input twice is in the domain once", nfail)
    ...
    do i = 1, sets % num_members_of(dup)
       do j = i + 1, sets % num_members_of(dup)
          passes = passes .and. (sets % member_of(dup, i) /= sets % member_of(dup, j))
       end do
    end do
    call report(passes, &
    ...
    passes = .true.
    do i = 1, sets % num_members_of(dup)
       passes = passes .and. (sets % member_of(dup, sets % index_in(dup, sets % member_of(dup, i))) &
            &         .eq. sets % member_of(dup, i))
       passes = passes .and. (sets % index_in(dup, sets % member_of(dup, i)) .eq. i)
    end do
    call report(passes, &
  ...
  ! checked by its own representation and returning its own identity.
  !===================================================================!
  ...
  subroutine check_mixed_carriers(nfail)
    ...
    call report(d % same_as(sensors), &
         & "the listed slot is the listed domain, by identity", nfail)
  ...
  !===================================================================!
  ! Adjacency is a view, not a primitive: the same carrier is
  ! in both slots, and the signature states so by identity.
  !===================================================================!
  ...
  ! x roles. An interior edge has two tuples, tail and head; the
  ! boundary edge has one, and NO tuple invents its far side.
  !===================================================================!
  ...
  subroutine check_ternary_endpoint(nfail)
    ...
    ! 1 --> 2 --> 3, then edge 3 leaves vertex 3 across the boundary.
    ...
    call report(ends % has([1, 1, 1]) .and. ends % has([1, 2, 2]), &
         & "an interior edge has its tail and its head", nfail)
    call report(ends % has([3, 3, 1]), &
         & "the boundary edge has its one tail", nfail)

    passes = .true.
    do v = 1, 4
       passes = passes .and. .not. ends % has([3, v, 2])
    end do
    call report(passes, &
         & "and no tuple invents a head across the boundary", nfail)
  ...
  subroutine check_empty_relation(nfail)
    ...
    call report(tk % declared() .and. none % num_tuples() .eq. 0, &
         & "the empty relation is declared and contains nothing", nfail)
