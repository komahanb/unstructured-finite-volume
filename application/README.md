...
- A continuous law is explicit in `src/operation_domain.f90`:
  ...
  graph's vertex set, from which it types the law's inputs (a state
  with the law's component count, a design with one value per point)
  and a functional's state. A residual types its own supports from its
  own graphs: state, direction and tangent on U, residual, costate and
  forcing on Y = U, design on P (`state_fields`, `residual_fields`,
  `design_fields` of `residual_operator`).
...
G18-G21 measure every family's order and the invariant's drift. The law
