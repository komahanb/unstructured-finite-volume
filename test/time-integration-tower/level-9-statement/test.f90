...
! arrives as one at the march digraph's source vertex; the driver
! places the result at the last state vertex as one, and its domain
! is checked. No plain vector crosses the statement's boundary.
!
...
! off the Level-5 coordinate field, four steps taken, and the
...
program time_level_9
  ...
  use temporal_march_fixture, only : marched, march_incidence, MARCH_BDF2
  ...
  type(stored_field)            :: q_initial, tcoord
  class(field), allocatable     :: q_final
  integer                :: nfail
  ...
  ! ---- the constitution, as Level 8 established it: bdf-2 driven
  !      over the march digraph, newton over gmres at every step,
  !      the initial field in and the final field out.
  q_final = marched(MARCH_BDF2, decay, H_STEP, NSTEPS, hcontext, q_initial, &
...
contains
  ...
  ! the last integer" but: four steps were taken, the control chain
  ...
  subroutine check_the_endpoint_is_earned(nfail)
    ...
    ! Traverse the march digraph's control chain from its first
    ! instant, following incidence, to where four steps land.
