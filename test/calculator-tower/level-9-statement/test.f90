...
contains
  ...
  subroutine cr_apply(this, input_graph, inputs, output)
    ...
    if (.not. state % defined_on(this % unknown)) then
       error stop 'statement: the state must live on the unknown domain'
    end if
    call bound_real_vector(inputs, this % argument(1), ustate)
...
program calculator_level_9
  ...
  call solver % state(residual_op, host, u, sets % num_members_of(u))
  ...
  ! the affine part is the residual of the zero state, evaluated when
  ! the solver is stated; R(q) = 0 is A q = -R(0)
  gv   = solver % affine
