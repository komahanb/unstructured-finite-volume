...
!
!      INDEPENDENT-AXIS STENCIL     family % block_connectivity()
!                                   BDF2's fan-in, 1->3 2->3 3->3
!
!      DEPENDENT-AXIS STENCIL       stencil % pattern
...
!
! The family is never applied and never solved. It is constructed,
! asked for its own axis, and set beside the others. A family takes no
! action and no step size (src commit b9944c3).
!
...
program visualization_level_8
  ...
  use production_discretization_fixture, only : dependency_pattern, temporal_pattern
  ...
  a = stencil(rows     = [1, 2, 3, 1, 2, 2, 3], &
       ...
       &               label    = 'A')
  call dependency_pattern(a, dependent)
  ...
  ! ---- the independent axis: BDF2, beside that action.
  clock = bdf_family(2)
  call temporal_pattern(clock, independent)
  ...
  euler = bdf_family(1)
  call temporal_pattern(euler, independent_be)
...
contains
  ...
  subroutine say_the_constitution()
    ...
    write(*,'(4x,a)') "INDEPENDENT-VARIABLE STENCIL   family % block_connectivity()"
    pic = pattern_picture(independent, '', sets, labels); call say_grid(pic)

    write(*,'(4x,a)') "DEPENDENT-VARIABLE STENCIL     stencil % pattern"
  ...
  !===================================================================!
  ! The independent axis is a stencil, and it is the family's own.
  !===================================================================!
