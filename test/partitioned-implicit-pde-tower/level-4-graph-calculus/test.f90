...
contains
  ...
  ! HOLDS members it does not OWN. The owned, borrowed and overlap
  ! subsets are declared here from the relation's owner map, as
  ! subobjects of the part's own vertex carrier: the directed view
  ! no longer declares them (bff4a06).
  !===================================================================!
  ...
  subroutine check_maps_and_ownership(part, kpart, globals, &
    ...
    type is (stored_directed_graph)
       ...
       call subsets % bind(part % vertex_set(), &
  ...
  ! The local vertices whose owner is this part (owned) or is not
  ! (borrowed), in local order.
  function owner_selected(part, rel, kpart, owned) result(members)
    ...
    allocate(members(0))
    do i = 1, part % num_vertices()
       if ((rel % vertex_owner_part(i) .eq. kpart) .eqv. owned) then
          members = [members, i]
       end if
    end do

  end function owner_selected
