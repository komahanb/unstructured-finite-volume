...
program calculator_level_4
  ...
  flow = stored_relation('flow', [o, x, p], table, sets % set_map)
...
contains
  ...
  subroutine check_sources_and_sinks(nfail)
    ...
    call report(sets % num_members_of(src) .eq. 1 .and. sets % has(src, OP_PLUS), &
         & "sources(D) = { + }, the selector long dead", nfail)
    call report(sets % num_members_of(snk) .eq. 1 .and. sets % has(snk, OP_TIMES), &
         & "sinks(D) = { x }", nfail)

    call report(sets % subobject_of(src, o), &
         & "the sources stand embedded in the operations", nfail)
    call report(sets % subobject_of(snk, o), &
  ...
  subroutine check_reachability(nfail)
    ...
    call report(reachable(d, sets % set_map, OP_PLUS, OP_TIMES), &
         & "reachable(+, x) = true", nfail)
    call report(.not. reachable(d, sets % set_map, OP_TIMES, OP_PLUS), &
         & "reachable(x, sets, +) = false", nfail)
    call report(reachable(d, sets % set_map, OP_PLUS, OP_PLUS), &
