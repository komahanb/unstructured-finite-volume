...
program derivative_level_4
  ...
  d = compose_binary( &
       & project_slots(restrict_slot(flow, 3, p_out, sets % set_map, inclusions), [1, 2], sets % set_map), &
       & project_slots(restrict_slot(flow, 3, p_in , sets % set_map, inclusions), [2, 1], sets % set_map), &
       & sets % set_map)
...
contains
  ...
  subroutine check_reachability(nfail)
    ...
    call report(reachable(d, sets % set_map, OP_PRODUCT, OP_SUM), &
         & "reachable(product, sets, sum) = true", nfail)
    call report(.not. reachable(d, sets % set_map, OP_SUM, OP_PRODUCT), &
         & "reachable(sum, sets, product) = false", nfail)
    call report(reachable(d, sets % set_map, OP_PRODUCT, OP_PRODUCT) .and. &
         &      reachable(d, sets % set_map, OP_SUM, OP_SUM), &
