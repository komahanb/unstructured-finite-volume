...
program test_graph_dense_direct
  ...
  call solver % state(one_by_one, one_by_one % pattern, &
  ...
  call solver % state(two_by_two, two_by_two % pattern, &
  ...
  call solver % state(pivoting, pivoting % pattern, &
...
contains
  ...
  subroutine column_of(statement, j, column)
    ...
    call statement % apply(statement % pattern, statement % bind([basis]), image)
