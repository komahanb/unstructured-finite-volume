...
program adjoint_level_4
  ...
  table(:, 8) = [TGT_F , VAR_U]
  table(:, 9) = [TGT_F , VAR_V]
  dep = stored_relation('dependency', [t, v], table, sets % set_map)

  inc_y   = inclusion_of('Y in T', y_dom, t, sets % set_map)
  inc_q   = inclusion_of('Q in V', q_dom, v, sets % set_map)
  inc_q_t = transpose_of(inc_q)
  jq      = compose_binary(compose_binary(inc_y, dep, sets % set_map), inc_q_t, sets % set_map)
  ...
  coupling = compose_binary(jq_t, jq, sets % set_map)
...
contains
  ...
  subroutine check_view_is_valid(nfail)
    ...
    dom = coupling % source()
    call report(dom % same_as(q_dom) .and. sets % num_members_of(dom) .eq. 2, &
  ...
  subroutine check_the_cycle(nfail)
    ...
    call report(reachable(coupling, sets % set_map, VAR_U, VAR_V) .and. &
         &      reachable(coupling, sets % set_map, VAR_V, VAR_U), &
  ...
  subroutine check_no_source_or_sink(nfail)
    ...
    call sources(coupling, sets, src)
    call sinks(coupling, sets, snk)

    call report(sets % num_members_of(src) .eq. 0 .and. sets % num_members_of(snk) .eq. 0, &
  ...
  ! nothing else. Built from the membership of S alone.
  !===================================================================!

  type(csr_relation) function inclusion_of(name, s, host, sets) &
       & result(inclusion)
    ...
    inclusion = csr_relation(name, s, host, table, sets)
