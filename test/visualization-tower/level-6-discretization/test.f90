...
! RELATIONALLY.
!
! This is the first level to name production machinery. The stencil's
! dependent-axis pattern is its stored pattern component; the family's
! independent-axis pattern is its block connectivity (src commits
! 9fd1732 and b9944c3).
!
...
!    stored pattern renders an identical grid to the relational
...
!
! 3. THE FAMILY WITNESS. The other concrete citizen is read on a
!    different AXIS: the stencil on the independent variable. BDF2's
!    residual at the newest instant reads three instants, so its
!    stencil is a FAN-IN - 1->3, 2->3, 3->3 - and not the succession
!    1->2->3, which is a chronology and a different relation entirely.
!
!    The family takes no action: its connectivity is read from the
!    family alone, so the independent axis cannot depend on any state
!    sparsity (src commit b9944c3).
!
!    ONE PATTERN PER AXIS: the stored pattern for the stencil
!    operator, the block connectivity for the family.
!
...
program visualization_level_6
  ...
  use production_discretization_fixture, only : dependency_pattern
  use production_discretization_fixture, only : temporal_pattern, bdf2_family
  ...
  type(stencil) :: sten_d2, sten_d1
  type(family)  :: clock_d2

  class(directed_graph), allocatable :: pat_d2, pat_d1
  class(directed_graph), allocatable :: motif_d2
  ...
  call dependency_pattern(sten_d2, pat_d2)
  ...
  call dependency_pattern(sten_d1, pat_d1)
  ...
  clock_d2 = bdf2_family()

  call temporal_pattern(clock_d2, motif_d2)
...
contains
  ...
  ! the right ones from production's own stored patterns.
  !===================================================================!
  ...
  subroutine check_the_stencil_coordinate_pattern(nfail)
    ...
    call report(pat_d2 % num_vertices() .eq. NX1 .and. &
         ...
         & "production's stored pattern is a graph of 3 vertices " // &
  ...
  !===================================================================!
  ! MEASUREMENT THREE. The other concrete citizen, on its own axis.
  !===================================================================!
  ...
  subroutine check_the_step_pattern(nfail)
    ...
    call report(motif_d2 % num_vertices() .eq. 3 .and. &
         ...
         & "BDF2's block connectivity has three vertices and three " // &
         & "edges - history depth + 1 instants, and one arrow from each", nfail)
  ...
  subroutine check_state_is_not_time(nfail)
    ...
    call report(motif_d2 % num_vertices() .eq. pat_d2 % num_vertices(), &
         & "the family's pattern and the stencil's pattern have " // &
    ...
    call report(production_has(pat_d2, 2, 1) .and. &
         ...
         & "family's independent axis has no such arrow", nfail)

    call report(production_has(motif_d2, 1, 3) .and. &
         &      .not. production_has(pat_d2, 1, 3), &
         & "the family's residual reads instant 1; the state stencil " // &
