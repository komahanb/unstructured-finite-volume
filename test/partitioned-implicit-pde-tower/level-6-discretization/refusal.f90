...
!
!   unequal-domain . a state field of exactly the RIGHT SIZE,
!                        living on a set that is not this
!                        graph's vertex set, must be refused
!
! The unequal domain holds six members, as V(G) does. Nothing but
...
program partitioned_pde_level_6_refusal
  ...
  case ('unequal-domain')
     ...
     unequal_domain = index_set('a set of the right size', NV)
     q = field('state on an unequal domain', unequal_domain)
     call q % set_real_vector(Q_EXACT)
     call shifted % apply(g, [q], out)
     write(*,*) 'a set of the right size but unequal identity was accepted'
