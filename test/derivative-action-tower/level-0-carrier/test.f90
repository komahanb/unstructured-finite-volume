...
contains
  ...
  subroutine check_cardinalities(nfail)
    ...
    call report(sets % num_members_of(v) .eq. 4, &
  ...
  subroutine check_membership_boundary(nfail)
    ...
    call report(sets % has(v, SLOT_X) .and. sets % has(v, SLOT_Z), &
         & "x and z are value slots", nfail)
    call report(sets % has(o, OP_PRODUCT) .and. sets % has(p, PORT_OUT), &
         & "product is an operation and out is a port", nfail)

    call report(.not. sets % has(v, 5) .and. .not. sets % has(v, 0), &
