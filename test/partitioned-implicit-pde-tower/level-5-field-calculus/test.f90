...
contains
  ...
  subroutine check_edge_assembly_law(nfail)
    ...
    allocate(total(NV))
    total = 0.0_dp
    z = stored_field('vertex probe', g % vertex_set(), g % num_vertices())
  ...
  subroutine check_one_transport(q, part, k, globals, expect, nfail)
    ...
    type is (stored_directed_graph)
       pvs = part % vertex_set()
       call report(dom % same_as(pvs) .and. sets % num_members_of(dom) .eq. 4, &
  ...
  subroutine check_vertex_round_trip(nfail)
    ...
    from1 = 0.0_dp
    from2 = 0.0_dp
    call add_round_trip(q, 1, sets, from1)
    call add_round_trip(q, 2, sets, from2)
  ...
  subroutine check_proper_subset_transport(nfail)
    ...
    call sets % name(s, 'subset probe')
    call sets % include_in(s, vs)
    d = stored_field('subset probe', s, 3)
    ...
    do k = 1, 2
       ...
       call fd % real_vector(v)
          call report(sets % subobject_of(dom, vs), &
  ...
  subroutine add_round_trip(d, k, sets, total)

    type(stored_field)        , intent(in)    :: d
    integer            , intent(in)    :: k
