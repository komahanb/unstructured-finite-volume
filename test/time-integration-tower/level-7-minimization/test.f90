...
! argument - `state(action, context, unknown_domain, num_unknowns)` -
! and reads the residual domain from the ACTION rather than the host. Its own
...
contains
  ...
  subroutine check_backward_euler_solve(nfail)
    ...
    call solver % state(step, ht, q, NQ, num_components=1, stored_inputs=[hist])
  ...
  subroutine check_bdf2_solve(nfail)
    ...
    call solver % state(step, ht, q, NQ, num_components=1, stored_inputs=[hist1, hist2])
  ...
  subroutine check_unknown_domain_is_the_caller_s_word(nfail)
    ...
    ! the action's own, which is why state agreed to exist at all:
