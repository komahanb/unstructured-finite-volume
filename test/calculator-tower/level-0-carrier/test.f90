...
contains
  ...
  subroutine check_cardinalities(nfail)
    ...
    call report(sets % num_members_of(x) .eq. 5, &
         & "X counts its five value slots", nfail)
    call report(sets % num_members_of(o) .eq. 2, &
         & "O counts its two operations", nfail)
    call report(sets % num_members_of(p) .eq. 3, &
  ...
  subroutine check_enumeration_round_trips(nfail)
    ...
    do i = 1, sets % num_members_of(x)
       satisfied = satisfied .and. (sets % member_of(x, sets % index_in(x, sets % member_of(x, i))) &
            &         .eq. sets % member_of(x, i))
       satisfied = satisfied .and. (sets % index_in(x, sets % member_of(x, i)) .eq. i)
    end do
    call report(satisfied, "member and local_index invert on X", nfail)
  ...
  subroutine check_membership_boundary(nfail)
    ...
    call report(sets % has(x, SLOT_A) .and. sets % has(x, SLOT_E), &
         & "a and e are value slots", nfail)
    call report(sets % has(o, OP_PLUS) .and. sets % has(p, PORT_OUT), &
         & "+ is an operation and out is a port", nfail)

    call report(.not. sets % has(x, 6) .and. .not. sets % has(x, 0), &
         & "an outsider is rejected by X", nfail)
    call report(.not. sets % has(o, 3), &
         & "and by O", nfail)
    call report(.not. sets % has(p, 4), &
         & "and by P", nfail)
