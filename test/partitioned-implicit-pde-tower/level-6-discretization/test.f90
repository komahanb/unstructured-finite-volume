...
contains
  ...
  subroutine check_global_action(nfail)
    ...
    call lap % apply(g, lap % bind([q]), lq)
    ...
    call shifted % apply(g, shifted % bind([q]), aq)
    dom = aq % domain()
  ...
  type(stored_field) function local_state(part, k, sets) &
       & result(qp)
  ...
  subroutine add_local_action(part, k, total)
    ...
    call a % assemble_data(rel, part, aq_local, g, sets, fd)
    dom = fd % domain()
