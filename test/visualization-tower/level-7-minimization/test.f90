...
!      minimizer % block_diagonal()   one indicator per colour
!      jacobi % solve()               consumes block_diagonal()
!      gauss_seidel % solve()         consumes block_diagonal() and
!                                     sweep_order()
!
...
!
!      P_A     the operator's own numerical coupling - the stored
!              pattern stencil % pattern, and what
...
!
! THE RESOLUTION WAS NOT TO ASK THE ACTION. A pattern is
! axis-relative - a stencil's is on the dependent variable, a family's
! on the independent one - so a minimizer reaching in for one would
! have coloured a BDF2 family's unknowns by a fan-in over instants.
...
!                 sweep_order, supplied EXPLICITLY when the solver is
!                 stated and never defaulted from `on`
!
! The caller knows which object owns the dependent axis. Here it is
! the stencil, and the stencil says so through its own stored
! pattern. Below, both statements are given coupling = P_A and differ
! only in `on` - so the diagonal must not move, and does not.
!
!                            THE EXPERIMENT
!
...
! matvec, sweep_order, block_diagonal - which is what makes the measurement
...
program visualization_level_7
  ...
  call diagonal_of(on_match, d_match)
  call diagonal_of(on_empty, d_empty)
...
contains
  ...
  !===================================================================!
  ! State a jacobi on the one stencil over a given host, using that
  ! host's own vertex carrier as the unknown domain.
  !===================================================================!

  subroutine state_over(solver, on)
    ...
    call solver % state(a, on, unknowns, sets % num_members_of(unknowns), coupling = pa)

  end subroutine state_over

  !===================================================================!
  ! The diagonal of the stated action: the block diagonal at block
  ! width one, one number per unknown.
  !===================================================================!

  subroutine diagonal_of(solver, d)

    type(jacobi), intent(in)           :: solver
    real(dp), allocatable, intent(out) :: d(:)

    real(dp), allocatable :: blocks(:,:,:)

    call solver % block_diagonal(blocks)
    d = blocks(1, 1, :)
  ...
  subroutine say_the_experiment()
    ...
    write(*,'(4x,a)') "OPERATOR DEPENDENCY P_A   (stencil % pattern)"
  ...
  ! whichever host the operator is later stated over - because it is
  ! the stencil's stored pattern and nothing else.
  !===================================================================!
  ...
  subroutine check_the_operator_owns_its_structure(nfail)
    ...
    allocate(again, source=a % pattern)
    ...
    call report(same_pattern(pa, again), &
         & "P_A DOES NOT DEPEND ON A HOST - it is the stencil's own " // &
         & "stored pattern, and reads the same twice", &
         & nfail)
