...
program adjoint_level_9
  ...
  inc_y = inclusion_of('Y in T', y_dom, t, sets)
  inc_z = inclusion_of('Z in T', z_dom, t, sets)
  inc_q = inclusion_of('Q in V', q_dom, v, sets)
  inc_p = inclusion_of('P in V', p_dom, v, sets)
...
contains
  ...
  subroutine solve_the_primal(nfail)
    ...
    call primal_solver % state(primal_eq, host, q_dom, sets % num_members_of(q_dom))
  ...
  subroutine solve_the_adjoint(nfail)
    ...
    call adjoint_solver % state(adjoint_eq, host, y_dom, sets % num_members_of(y_dom))
    ...
    call adjoint_solver % apply(host, adjoint_solver % bind([rhs_q]), sol)
  ...
  subroutine certify_with_the_tangent(nfail)
    ...
    call tangent_solver % state(tangent_eq, host, q_dom, sets % num_members_of(q_dom))
    tangent_solver % tolerance      = 1.0d-12
    tangent_solver % max_iterations = 50

    gv = tangent_solver % affine
    rhs_y = stored_field('rhs', y_dom, sets % num_members_of(y_dom))
    call rhs_y % set_real_vector(-gv)
    call tangent_solver % apply(host, tangent_solver % bind([rhs_y]), sol)
