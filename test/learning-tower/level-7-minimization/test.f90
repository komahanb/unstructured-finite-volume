...
!
! The level checks one question: GIVEN a supplied residual map
...
! state, affine, solve - which discovers the linear action
...
module learning_residual_fixture
  ...
  use operation_action, only : operation, binding, bound_value, emit_real
  ...
  type, extends(operation) :: affine_learning_residual
     ...
     ! representation has no identity, so storing one keeps this
...
contains
  ...
  subroutine oracle_apply(this, input_graph, inputs, output)
    ...
    if (.not. present(inputs)) then
       error stop 'oracle: the residual is evaluated at a state'
    end if
    ...
    call emit_real('residual', this % y, this % n_y, r, output)
...
program learning_level_7
  ...
  call solver % state(oracle, host, theta, sets % num_members_of(theta))
  solver % tolerance      = 1.0d-12
  solver % max_iterations = 50

  call solver % domain(host, dom, n_dom)
  call report(dom % same_as(theta), &
       & "the solver's solution domain is Theta, by identity", nfail)
  call oracle % domain(host, dom, n_dom)
  call report(dom % same_as(y), &
       & "and the oracle's residual lives on Y", nfail)
  ...
  ! w0 = 0: Level 5's initial parameter, the same in meaning.
  w_state = [0.0_dp]
  ...
  call oracle % apply(host, oracle % bind([state]), resid)
  ...
  call fitter % state(witness, host, theta, sets % num_members_of(theta))
