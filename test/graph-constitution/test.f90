...
contains
  ...
  subroutine fidelity_case(m, bc, a, b, c, area, delta, kappa, vn, name, nfail)
    ...
    call bc % coefficient_values(m, kappa, COEFFICIENT_LHS, got)
  ...
  subroutine check_operator_road(nfail)
    ...
    with_wall = differential_operator(SIDE_VERTEX, 2, coefficients=c, &
         & spacings=[0.8_dp, 0.3_dp, 0.6_dp], boundary_values=b)
    call with_wall % apply(m, with_wall % bind([state]), y)
  ...
  subroutine check_the_wall_keeps_both_numbers(nfail)
    ...
    type(mesh) :: m
    type(stencil) :: op
    real(dp), allocatable :: x(:), mixed(:), pinned(:)
    real(dp) :: achieved
