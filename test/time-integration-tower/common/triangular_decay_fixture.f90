...
contains
  ...
  type(triangular_decay) function create_decay(state, n_state) result(this)
    ...
    ! the operation reads one input, its state: one real number per
    ! coordinate
  ...
  subroutine decay_apply(this, input_graph, inputs, output)
    ...
    call emit_real('triangular decay', this % state, this % n_state, s, output, &
         & num_components=1)
