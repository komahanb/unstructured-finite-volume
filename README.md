...
is partitioned (src/class_partition.f90), each image owns its rows, the matvec
exchanges only the halo over coarrays and the dot products reduce with co_sum;
one image reduces exactly to the serial CG. a per-image AMG on the owned block
is a restricted-additive-schwarz preconditioner. run it with:
