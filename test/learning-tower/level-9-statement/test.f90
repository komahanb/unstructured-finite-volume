...
!
! The residual operation is a test-local adapter storing the
! GRAPH-OWNED flow (the external selector is deallocated before the solve),
...
program learning_level_9
  ...
  ! -- the adapter keeps the GRAPH-OWNED flow; the selector is deallocated.
  residual_op = constituted_learning_residual(g, bnd, flow, located, &
       & v, y, sets, k, obs, theta, u, order)
  deallocate(flow)

  ! -- the compatibility host: seven vertices, nobody's trainables
  ...
  call solver % apply(host, solver % bind([rhsf]), sol)
  ...
  type is (stored_field)
     call residual_op % apply(host, residual_op % bind([sol]), rf)
  end select
...
contains
  ...
  ! and the role subdomains keep their extensions - checked both
  ...
  subroutine check_inference(nfail)
    ...
    ! values: the trainable slot takes the learned state, the
    ! other takes the new input.
