...
program horizon_scaling
  ...
  use util_tally            , only : tally, newton_solves, linear_solves
...
contains
  ...
  real(dp) function counted(account, event)
    ...
    do level = 1, account % num_levels()
       do order = 0, max_order + 1
          counted = counted + account % amount(level, order, event)
       end do
    end do
  ...
  subroutine reverse_horizon()
    ...
    call context % account % open(max_order + 1, hierarchy_levels)
    ...
    call record(tokens, 'initialize', measured)
    advances = 0
    before_newton = counted(execution % account(), newton_solves)
    before_linear = counted(execution % account(), linear_solves)
    call begin_phase(measured)
    do while (.not. execution % complete())
       call execution % advance()
       advances = advances + 1
    ...
    call end_phase(measured)
    call record(tokens, 'advance', measured)
    march_newton = counted(execution % account(), newton_solves) - before_newton
    march_linear = counted(execution % account(), linear_solves) - before_linear
    before_newton = counted(execution % account(), newton_solves)
    before_linear = counted(execution % account(), linear_solves)
    call begin_phase(measured)
    ...
    reverse_newton = counted(execution % account(), newton_solves) - before_newton
    reverse_linear = counted(execution % account(), linear_solves) - before_linear
