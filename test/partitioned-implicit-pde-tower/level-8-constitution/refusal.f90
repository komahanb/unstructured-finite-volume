...
!   foreign-host-attach .... gmres.state(A_part, G_alt, V(G_alt))
!
...
program partitioned_pde_level_8_refusal
  ...
  case ('foreign-host-apply')
     ...
     call composite % apply(g_alt, composite % bind([q]), out)
     write(*,*) 'a chain decomposition acted on the star'

  case ('foreign-host-attach')
     ! state asks the action for its domain; the guard fires there.
     call solver % state(composite, g_alt, g_alt % vertex_set(), g_alt % num_vertices())
