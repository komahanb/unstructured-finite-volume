...
allowed_for() {
    case "$1" in
        common)                  echo "" ;;
        level-0-set)         echo "learning_assert graph_set" ;;
        level-1-relation)        echo "learning_assert graph_set graph_relation" ;;
        # level 2: the algebra, and NOT the binary storage - D is
        # held as class(relation).
        level-2-relation-algebra) echo "learning_assert graph_set graph_relation graph_relation_algebra" ;;
        # level 5: values need domains, not graphs - the smallest
        # allowlist of any rung above the ground.
        level-5-field-calculus)  echo "learning_assert graph_set class_graph_field" ;;
        ...
        level-4-graph-calculus)  echo "learning_assert graph_set graph_relation graph_relation_algebra graph_structure graph_interpretation graph_algorithms" ;;
        # level 3: the container; graph_binary_relation is granted
        # for the view refusal ONLY - the production path never
        # touches it.
        level-3-graph)           echo "learning_assert graph_set graph_relation graph_relation_algebra graph_structure graph_binary_relation" ;;
        ...
        level-6-discretization)  echo "learning_assert graph_set graph_relation graph_relation_algebra graph_binary_relation graph_structure graph_interpretation graph_algorithms" ;;
        ...
        # the list; the relation/algebra/interpretation stack is not needed
        # at all - the solver sees an opaque R : Theta -> Y. The
        # named fixture is the level's own test-local oracle.
        level-7-minimization)    echo "learning_assert graph_set graph_grammar class_graph_field class_graph class_graph_gmres learning_residual_fixture" ;;
        ...
        level-8-constitution)    echo "learning_assert graph_set graph_relation graph_relation_algebra graph_structure graph_interpretation graph_algorithms class_graph_field learning_constitution_fixture" ;;
        ...
        level-9-statement)       echo "learning_assert graph_set graph_relation graph_relation_algebra graph_structure graph_interpretation graph_algorithms graph_grammar class_graph class_graph_field class_graph_gmres learning_constitution_fixture constituted_residual_fixture" ;;
