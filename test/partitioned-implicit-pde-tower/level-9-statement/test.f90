...
contains
  ...
  subroutine check_solver_matvec_equivalence(nfail)
    ...
    call solver_global % state(direct, g, vs, n_vs)
    call solver_part % state(composite, g, vs, n_vs)
  ...
  subroutine check_the_two_solves(nfail)
    ...
    call solver_part % apply(g, solver_part % bind([rhs]), sol)
    dom = sol % domain()
    ...
    call solver_global % apply(g, solver_global % bind([rhs]), sol)
    call sol % real_vector(v)
    q_global = v(1:NV)
