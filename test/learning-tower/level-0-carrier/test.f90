...
contains
  ...
  subroutine check_cardinalities(nfail)
    ...
    call report(sets % num_members_of(v) .eq. 5, &
  ...
  subroutine check_membership_boundary(nfail)
    ...
    call report(sets % has(v, SLOT_W) .and. sets % has(v, SLOT_E), &
         & "w and e are value slots", nfail)
    call report(sets % has(o, OP_PREDICT) .and. sets % has(p, PORT_OUT), &
         & "predict is an operation and out is a port", nfail)

    call report(.not. sets % has(v, 6) .and. .not. sets % has(v, 0), &
