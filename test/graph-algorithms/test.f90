!=====================================================================!
! The graph algorithms suite: the laws of the four traversals
! (level 4) over the directed adjacency interpretation, checked
! generically, apart from the calculator that first required them.
!
! The domain is built to expose ordering by member value: its
! declaration order is [3, 1, 5, 4, 2], so every canonical result
...
program test_graph_algorithms
  ...
  call check_sources_and_sinks(sets, nfail)
  call check_reachability(sets % set_map, nfail)
  call check_topological_order(sets % set_map, nfail)
  call check_tuple_order_invariance(v, sets, nfail)
...
contains
  ...
  ! DOMAIN'S declaration order - and the isolated member 5 is a
  ! member of both.
  !===================================================================!

  subroutine check_sources_and_sinks(sets, nfail)
    ...
    call report(sets % subobject_of(src, v) .and. sets % subobject_of(snk, v), &
         & "and both results are subobjects of the domain", nfail)
  ...
  subroutine check_reachability(sets, nfail)
    ...
    call report(.not. reachable(after, sets, 7, 2) .and. &
         ...
         & "an outsider endpoint is false, never an invalid index", nfail)

  end subroutine check_reachability

  !===================================================================!
  ! One graph, one order: the deterministic Kahn traversal returns
  ! [3, 1, 5, 4, 2] - ties broken by declaration order, never by
  ! member value.
  !===================================================================!
  ...
  ! the same extension listed backwards has the same sources,
  ! sinks, reachability and order.
  !===================================================================!

  subroutine check_tuple_order_invariance(v, sets, nfail)
    ...
    call report(reachable(backwards, sets % set_map, 3, 2) .and. &
         &      .not. reachable(backwards, sets % set_map, 2, 3), &
         & "and so does reachability", nfail)

    call topological_order(backwards, sets % set_map, order)
