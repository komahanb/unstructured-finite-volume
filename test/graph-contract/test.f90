...
contains
  ...
  subroutine nonlinear_sample_apply(this, input_graph, inputs, output)
...
!      recorded.
!   9. The named sets. The two carriers list every member, a tag
!      names its edges, and a tag present nowhere returns an empty
...
!  11. An uncut graph reports no partition record, and its identity
!      relation treats it as the whole of itself - one part,
...
program test_graph_contract
  ...
  use operation_traversal , only : traversal, TRAVERSAL_COLOURING, TRAVERSAL_VISIT_ORDER
  use operation_traversal , only : TRAVERSAL_COMPONENT, TRAVERSAL_DEPTH
...
contains
  ...
  subroutine describe(sets, g)
    ...
    class(set_map), intent(inout) :: sets
  ...
  !===================================================================!
  ! The named sets: the two carriers and the tagged edges.
  !===================================================================!
  ...
  subroutine check_graph_uncut(nfail)
    ...
    call owned_indices(grel, g, 1, .true., indices)
    call report(size(indices) .eq. 4, "and it owns every cell in it", nfail)

    call owned_indices(grel, g, 1, .false., indices)
  ...
  subroutine check_partition_covers_once(nfail)
    ...
    do k = 1, 2
       ...
       call owned_indices(rel, part, k, .true., indices)
       do l = 1, size(indices)
       ...
       call owned_indices(rel, part, k, .false., indices)
  ...
  subroutine check_operation_consistency(nfail)
    ...
    do k = 1, 2
       ...
       call sets       % include_in(owned_only(k), part % vertex_set())
  ...
  subroutine check_balance_conserves(nfail)
    ...
    call q % set_real_vector([1.0_dp, 2.0_dp, 3.0_dp, 4.0_dp])
    call bal % apply(open_chain, bal % bind([q]), y)
    call y % real_vector(v)
    call report(abs(sum(v)) > 1.0d-12, &
  ...
  subroutine check_walks(nfail)
    ...
    w = traversal(TRAVERSAL_COLOURING)
    ...
    w = traversal(TRAVERSAL_DEPTH, seed=1)
    call w % apply(g, output=f)
    call f % integer_vector(c)
    ...
    w = traversal(TRAVERSAL_VISIT_ORDER, seed=1)
    call w % apply(g, output=f)
    call f % integer_vector(c)
    ...
    w = traversal(TRAVERSAL_COMPONENT)
    call w % apply(g, output=f)
    call f % integer_vector(c)
  ...
  subroutine check_differential_operators(nfail)
    ...
    op = differential_operator(SIDE_VERTEX, 0, coefficient=3.0_dp)
    ...
    op = divergence()
    call op % apply(ring, op % bind([zf]), yf)
    call yf % real_vector(y)
    ...
    op = differential_operator(SIDE_VERTEX, 2, &
  ...
  subroutine check_adjoints(nfail)
    ...
    fwd = differential_operator(SIDE_VERTEX, 1, coefficient=1.5_dp)
    rev = differential_operator(SIDE_VERTEX, 1, coefficient=1.5_dp, adjoint=.true.)

    call fwd % apply(g7, fwd % bind([qf]), yf)
    call yf % real_vector(aq)
    call rev % apply(g7, rev % bind([pf]), yf)
    ...
    call rev % apply(g7, rev % bind([qf]), yf)
    call yf % real_vector(aq)
    call fwd % apply(g7, fwd % bind([pf]), yf)
    call yf % real_vector(ap)
  ...
  subroutine check_curl_on_border_graph(nfail)
    ...
    call reduce_edges % apply(border, reduce_edges % bind([zf]), yf)
    call yf % real_vector(y)
  ...
  subroutine check_components(nfail)
    ...
    op = laplacian()
    call op % apply(g7, op % bind([qf]), yf)
    ...
    fwd = differential_operator(SIDE_VERTEX, 1, coefficient=2.0_dp)
    rev = differential_operator(SIDE_VERTEX, 1, coefficient=2.0_dp, adjoint=.true.)
  ...
  subroutine check_compiled_stencil_agrees(nfail)
    ...
    call op % apply(chain, op % bind([qf]), ya)
    call ya % real_vector(a)

    compiled = stencil_of(op, chain)
    call compiled % apply(chain, compiled % bind([qf]), yb)
    ...
    rev = differential_operator(SIDE_VERTEX, 2, &
         & coefficients=[2.0_dp, 5.0_dp, 1.0_dp, 4.0_dp, 3.0_dp], &
         & adjoint=.true.)

    call rev % apply(chain, rev % bind([qf]), ya)
    call ya % real_vector(a)
  ...
  subroutine check_adjoint_is_the_transpose(nfail)
    ...
    do order = 1, 4
       ...
       do j = 1, 5
          ...
          call fwd % apply(g, fwd % bind([unit]), yf)
          call yf % real_vector(col)
          a(:, j) = col

          call rev % apply(g, rev % bind([unit]), yf)
          call yf % real_vector(col)
          at(:, j) = col
       end do
  ...
  subroutine check_nonlinear_edge_formulas(nfail)
    ...
    call formula % apply(ring, formula % bind([qf]), zf)
    ...
    reduce_edges = divergence()
    call reduce_edges % apply(ring, reduce_edges % bind([samples]), yf)
    call yf % real_vector(y)
    ...
    reduce_edges = differential_operator(SIDE_VERTEX, 2, coefficients=c)
    call reduce_edges % apply(ring, reduce_edges % bind([qf]), yf)
    call yf % real_vector(y)
  ...
  subroutine check_inner_products(nfail)
    ...
    slope = gradient()
    call slope % apply(chain, slope % bind([uf]), work_e)
  ...
  !-------------------------------------------------------------------!
  ! The part-local vertex indices a piece owns (owned true) or borrows
  ! (owned false), read from the relation's ownership.
  !-------------------------------------------------------------------!

  subroutine owned_indices(rel, part, k, owned, indices)
    ...
    integer, allocatable    , intent(out) :: indices(:)
    integer :: v
    indices = pack([(v, v = 1, part % num_vertices())], &
         & [((rel % vertex_owner_part(v) == k) .eqv. owned, v = 1, part % num_vertices())])
  end subroutine owned_indices
