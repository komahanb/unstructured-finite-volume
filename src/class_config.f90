...
contains
  ...
  type(config) function create(filename) result(this)
    ...
    do iline = 1, size(lines)
       ...
       case ("mesh")
          this % meshfile = string(trim(tok(2) % str))

       case ("equation")
          this % equation = string(trim(tok(2) % str))

       case ("kappa")
          this % kappa = tok(2) % asreal()
       ...
       case ("omega")
          this % omega = tok(2) % asreal()

       case ("max_tol")
          this % max_tol = tok(2) % asreal()
       ...
       case ("t_init")
          this % tinit = tok(2) % asreal()

       case ("t_final")
          this % tfinal = tok(2) % asreal()

       case ("output")
          this % output = string(trim(tok(2) % str))
  ...
  subroutine print(this)
    ...
    do i = 1, this % nbc
       write(*,'(1x,a,a,1x,a,3es12.4)') &
            & "boundary : ", trim(this % bc_name(i) % str), &
