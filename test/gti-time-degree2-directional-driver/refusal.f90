!=====================================================================!
! The refusals that must die at the degree-2 seat, one per
! invocation:
!
...
!      firstsize        a first-derivative array shorter than the
!                       vertices
!      secondsize       a second-derivative array likewise
...
program refusal
  ...
  use gti_time_degree2_directional_drivers, only : &
       ...
       & gti_time_degree2_relation_result, &
  ...
  type(gti_time_degree2_relation_result)     :: step
  ...
  type(gti_value_buffer) :: vf(2), vs(2), vf_short(1)
  integer :: v
  ...
  case ('firstsize')
     ...
     call driver % solve_relation(r_form, time_graph, 1, design, eta, &
          & vf_short, vs, options, step)

  case ('secondsize')

     call driver % solve_relation(r_form, time_graph, 1, design, eta, &
          & vf, vf_short, options, step)
  ...
  case ('badseed')
     ...
     call vf(1) % set_real([1.0_dp, 2.0_dp])
     call driver % solve_relation(r_form, time_graph, 1, design, eta, &
          & vf, vs, options, step)
